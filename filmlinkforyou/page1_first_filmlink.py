	...
	for l in tag_hotserver:
	    ...
	    next_a = l.find_next("a")
	        ...
	        f.write(",".join(map(self.my_strip, [line_list[0], line_list[-1], 
                                                     video_type, next_a.get_text(), next_a.get("href"),  movie_image])) +  "\n")
                logging.debug([line_list[0], line_list[-1], video_type, next_a.get_text(), next_a.get("href"), movie_image])
